...
def draw_peaks(tag, list_of_peaks):
    ...
    path_in = cfg_max3.dir_debug + '/' + tag + '.png'
    ...
    for item in list_of_peaks:
        y = int(item[0])
        x = int(item[1])
        draw_peak(magn, x, y, cfg_max3.black, 2, -1)
        if n > 0:
            draw_text(magn, x + 5, y + 0, cfg_max3.black, str(n))
        else:
            draw_text(magn, x - 7, y + 23, cfg_max3.black, str(n))
        n += 1
    # -----------------------------------------

    path_out = cfg_max3.dir_debug + '/' + tag + '_peaks.png'
